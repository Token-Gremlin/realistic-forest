...
function clearNearClutter(f) {
  ...
  if (!clutter) return { plants: 0, sedges: 0, nearSedges: 0 };
  const p = f.camera.position;
  const fwd = p.clone();
  f.camera.getWorldDirection(fwd);
  const hide = new Set([
    'fern', 'bush', 'bramble', 'vine', 'herb',
    'mushroom', 'flower', 'rock', 'log', 'limb', 'lily',
  ]);
  let dropped = 0, sedges = 0, nearSedges = 0;
  for (const k of clutter.kinds) {
    if (k.arch.key === 'sedge') {
      for (const v of k.variants) {
        sedges += v.bucket.count;
        const d = v.bucket.data;
        for (let i = 0; i < v.bucket.count; i++) {
          const o = i * 12;
          const dist = Math.hypot(d[o] - p.x, d[o + 2] - p.z);
          const dx = d[o] - p.x, dy = d[o + 1] - p.y, dz = d[o + 2] - p.z;
          const facing = (dx * fwd.x + dy * fwd.y + dz * fwd.z) / (Math.hypot(dx, dy, dz) || 1);
          if (dist < 10 && facing > -0.05) nearSedges++;
        }
      }
    }
  ...
  return { plants: dropped, sedges, nearSedges };
}
...
return {
  ...
  sedges: cleared.sedges,
  nearSedges: cleared.nearSedges,
  treesDropped,
