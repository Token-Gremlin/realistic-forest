function catchUp(f) {
  ...
  if (f.forest.clutter) {
    ...
    for (let i = 0; i < 20; i++) f.forest.clutter.update(0.016, f.camera);
  }
  if (f.forest.water) {
...
function clearNearClutter(f) {
  ...
  if (!clutter) return { plants: 0, flowers: 0, nearFlowers: 0 };
  const p = f.camera.position;
  const fwd = p.clone();
  f.camera.getWorldDirection(fwd);
  const hide = new Set([
    'fern', 'bush', 'bramble', 'vine', 'sedge',
    'mushroom', 'log', 'limb', 'lily',
  ]);
  let dropped = 0, flowers = 0, nearFlowers = 0;
  for (const k of clutter.kinds) {
    if (k.arch.key === 'flower') {
      for (const v of k.variants) {
        flowers += v.bucket.count;
  ...
  return { plants: dropped, flowers, nearFlowers };
}
...
return {
  ...
  flowers: cleared.flowers,
  nearFlowers: cleared.nearFlowers,
  treesDropped,
